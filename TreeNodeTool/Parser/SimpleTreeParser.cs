...
namespace UnityRPG
{
	...
	public class SimpleTreeParser
	{
		...
		private static ITreeNodeContent getTreeNodeContentFromStr (string contentStr, TreeType treeType)
		{
			...
			switch (treeType) {
			case TreeType.World:
				return new WorldNodeContent () {
					linkIndex = Int64.Parse (contentList [0]),
					zoneName = contentList [1],
					description = contentList [2],
					avatar = contentList [3],
					x = Int32.Parse (contentList [4]),
					y = Int32.Parse (contentList [5])
				};
			case TreeType.Zone:
			...
			case TreeType.Dialog:
				return new DialogNodeContent () {
					linkIndex = Int64.Parse (contentList [0]),
					speaker = contentList [1],
					portraitSpritesheetName = contentList [2],
					portraitSpritesheeteIndex = Int32.Parse (contentList [3]),
					text = contentList [4]
				};
			case TreeType.Quest:
		...
		private static BattleNodeContent getBattleNodeContentFromStr (string contentStr)
		{
			...
			case BattleNodeType.Info:
				return new BattleNodeContent () {
					linkIndex = Int64.Parse (contentList [0]),
					nodeName = contentList [1],
					nodeType = battleNodeType,
					description = contentList [3],
					icon = contentList [4]
				};
			case BattleNodeType.Enemy:
				return new BattleNodeContent () {
					...
					description = contentList [3],
					icon = contentList [4],
					x = Int32.Parse (contentList [5]),
					y = Int32.Parse (contentList [6])
				};
			case BattleNodeType.Loot:
				return new BattleNodeContent () {
					...
					description = contentList [3],
					icon = contentList [4],
					count = Int32.Parse (contentList [5])
				};
		...
		public static StoreNodeContent getStoreNodeContentFromStr (string contentStr)
		{
			...
			case StoreNodeType.Info:
				return new StoreNodeContent () {
					linkIndex = Int64.Parse (contentList [0]),
					nodeType = storeNodeType,
					storeName = contentList [2],
					storePortrait = contentList [3],
					storeDialog = contentList [4]
				};
			case StoreNodeType.ItemClass:
				return new StoreNodeContent () {
					linkIndex = Int64.Parse (contentList [0]),
					nodeType = storeNodeType,
					itemType = getItemTypeFromStr (contentList [2]),
					buyPrice = float.Parse (contentList [3]),
					sellPrice = float.Parse (contentList [4]),
					count = Int32.Parse (contentList [5])
				};
			...
			foreach (var actionStr in actionSplitList) {
				...
				if (fieldSplitList.Count > 2) {
					TreeNodeAction tempAction = new TreeNodeAction () {
						actionType = getNodeActionTypeFromStr (fieldSplitList [0]),
						index = Int64.Parse (fieldSplitList [1]),
						actionName = fieldSplitList [2],
						count = Int32.Parse (fieldSplitList [3])
					};
